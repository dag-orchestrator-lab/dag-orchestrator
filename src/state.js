...
export function getPipelineStatus(cwd = process.cwd()) {
  ...
  if (state.hasTasks) {
    ...
    const totalMatches = tasksText.match(/###?\s+(?:\[[ x]\]\s*)?(T-\d+|Task\s+\d+)|\- \[[ x]\]\s+(Task|T-)\s*\d+/gi) || [];
    const doneMatches = tasksText.match(/(?:###?\s+\[x\]\s*(?:T-\d+|Task\s+\d+)|\- \[x\]\s+(?:Task|T-)\s*\d+|###?\s+T-\d+.*(?:\(DONE\)|\[DONE\]|—\s*DONE)|status:\s*(?:done|completed))/gi) || [];
